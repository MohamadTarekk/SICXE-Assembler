...
public class ErrorChecker {
    ...
    private int checkInstructionRestricted(SourceReader.CommandInfo ci,int lineNum){
        ...
        //18 spaces
        if(!operand.equals("                  ")&&!checkCanHaveOperand(operand))return STATEMENT_CANT_HAVE_OPERAND;
        if(checkIfWrongOperationPrefix(command))return WRONG_OPERATION_PREFIX;
        if(commandsMap.get(command)==null)return UNRECOGNIZED_OPERATION_CODE;
        if(checkIfUndefinedSymbolInOperand(operand))return UNDEFINED_SYMBOL_IN_OPERAND;
        if(ci.getTypeOperand().get(lineNum).equals("X")&&!checkIfHexadecimalString(ci.getOperand2().get(lineNum)))return NOT_HEXADECIMAL_STRING;
        //TODO ? SHOULD IT BE OPERAND ?
        if(checkIfIllegalAddressForRegister(operand)) return ILLEGAL_ADDRESS_FOR_REGISTER;
        if(checkIfItsFormat4Instruction(command))return CANT_BE_FORTMAT4_INSTRUCTION;



        if(lineNum==ci.getWholeInstruction().size()&&!command.equals("END"))return MISSING_END_STATEMENT;
    ...
    private boolean checkIfWrongOperationPrefix(String input){return true;}
    private boolean checkIfUndefinedSymbolInOperand(String input){return  true;}
    ...
    private boolean checkIfIllegalAddressForRegister(String input){return true;}
    private boolean checkIfItsFormat4Instruction(String input){return true;}
}
